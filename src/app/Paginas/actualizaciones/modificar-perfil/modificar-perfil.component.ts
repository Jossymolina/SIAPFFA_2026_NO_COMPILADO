...
arreglounidadEjecutora = []
  constructor(private _Activatedroute: ActivatedRoute, public _DatospersonalesService: ServicioBackendService,
    ...
    this._Activatedroute.params.subscribe(prm => {
      ...
      this.desplegar = parseInt(prm['id']);
...
this._ServiciosMensajeService.show()
    this._DatospersonalesService.consultaPorIdentidad(this.consulta).subscribe(
      Response => {
        ...
        this._ServiciosMensajeService.hide()
        this._ServiciosMensajeService.mensajeMalo("ERROR DE CONEXION AL REALIZAR LA CONSULTA")
      }
    )
  }
  atras(data) {
  ...
  GuardarCupos(formulario) {
    if (formulario.valid) {
      ...
      var parametro = {
        ...
        fechaIngreso: this.agregarNuevoCupo.fechaIngreso,
        usuario: this.usuarioLoguiado,
...
this.puestoSeleccionado = data
  ...
  displayCambiarFoto = false;
displayCambiarIdentidad = false;
displayCuentaBanco = false;
displayArma = false;
displayFechaIngreso = false;
...
abrirModalFechaNacimiento() { this.displayFechaNacimiento = true; }
